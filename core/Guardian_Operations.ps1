...
function Invoke-GuardianJob {
    ...
    try {
        ...
        if (Get-Command New-GuardianEvent -ErrorAction SilentlyContinue) {
            New-GuardianEvent -Source 'operations' -Category SYSTEM -Severity ERROR -Description "JOB_FAILURE: $Name - $($_.Exception.Message)" -AffectedComponent 'scheduler' | Write-GuardianEvent | Out-Null
        }
...
function Invoke-GuardianSchedulerCycle {
    ...
    $raw = Get-Content -Path $GuardianSchedulerState -Encoding UTF8 | ConvertFrom-Json
    $fresh = [PSCustomObject]@{
        jobs=$raw.jobs; lastCycle=$raw.lastCycle; created=$raw.created
    }
    $okCount = @($ran | Where-Object { $_.status -eq 'ok' }).Count
    $failCount = @($ran | Where-Object { $_.status -eq 'failed' }).Count
    $cycleResult = if ($failCount -eq 0) { 'ok' } else { 'degraded' }
    $fresh | Add-Member -MemberType NoteProperty -Name lastCycle -Value ($now.ToString('o')) -Force
    $fresh | Add-Member -MemberType NoteProperty -Name cycleDurationMs -Value ([math]::Round(((Get-Date) - $now).TotalMilliseconds, 1)) -Force
    $fresh | Add-Member -MemberType NoteProperty -Name cycleResult -Value $cycleResult -Force
    $fresh | Add-Member -MemberType NoteProperty -Name cycleSummary -Value ([PSCustomObject]@{ jobsRun=$ran.Count; ok=$okCount; failed=$failCount }) -Force
...
function Invoke-GuardianHealthScan {
    ...
    if (Get-Command New-GuardianEvent -ErrorAction SilentlyContinue) {
        New-GuardianEvent -Source 'operations' -Category SYSTEM -Severity INFO -Description "HEALTH_SCAN ok overall=$($health.overallPct)" -AffectedComponent 'health' | Write-GuardianEvent | Out-Null
    }
    return $res
}
...
function Invoke-GuardianCheckpointValidation {
    ...
    # Lifecycle retention (report-only here; archive handled by guarded operator action).
    Invoke-GuardianCheckpointLifecycle -KeepRolling 25 | Out-Null
...
function Invoke-GuardianHeartbeat {
    ...
    if (Get-Command New-GuardianEvent -ErrorAction SilentlyContinue) {
        New-GuardianEvent -Source 'operations' -Category SYSTEM -Severity INFO -Description 'GUARDIAN_ALIVE' -AffectedComponent 'operations' -Metadata @{ health=$health.overallPct; activeJobs=$activeJobs } | Write-GuardianEvent | Out-Null
    }
    ...
    # Governed communication: emit a structured health report to Nexus98 (no state mutation).
    if (Get-Command Send-GuardianHealthReportToNexus98Bridge -ErrorAction SilentlyContinue) { Send-GuardianHealthReportToNexus98Bridge | Out-Null }
...
function Get-GuardianHeartbeatStatus {
    ...
    $candidates = @(Get-Content -Path $GuardianHeartbeatLog -Encoding UTF8 | Where-Object { $_.Trim() })
    for ($k = $candidates.Count - 1; $k -ge 0; $k--) {
        try { return ($candidates[$k] | ConvertFrom-Json) } catch { continue }
    }
    return $null
}
# ---------------------------------------------------------------------------
...
function Start-GuardianOperations {
    ...
    try {
        ...
        if (Get-Command New-GuardianEvent -ErrorAction SilentlyContinue) {
            New-GuardianEvent -Source 'operations' -Category SYSTEM -Severity ERROR -Description "OPERATIONS_LOOP_FAILURE: $($_.Exception.Message)" -AffectedComponent 'operations' | Write-GuardianEvent | Out-Null
        }
    }
...
# ---------------------------------------------------------------------------
# Phase 7b: Runtime configuration validation.
# Every subsystem must be configurable and validated; invalid values are
# rejected rather than silently applied. Centralized in guardian_runtime_config.json.
# ---------------------------------------------------------------------------
function Test-GuardianRuntimeConfig {
    param([Parameter(Mandatory)]$Config)
    $errors = @()
    if ($Config -isnot [PSCustomObject] -and $Config -isnot [hashtable]) {
        return [PSCustomObject]@{ valid=$false; errors=@('config must be an object') }
    }
    if ($Config.PSObject.Properties.Name -contains 'schedulerIntervals') {
        $Config.schedulerIntervals.PSObject.Properties | ForEach-Object {
            $v = $_.Value
            if (-not ($v -is [int]) -or $v -lt 5 -or $v -gt 86400) {
                $errors += ("schedulerIntervals.{0} must be integer in [5,86400], got: {1}" -f $_.Name, $v)
            }
        }
    }
    if ($Config.PSObject.Properties.Name -contains 'retentionRules') {
        @('heartbeatDays','acksDays','eventsMax') | ForEach-Object {
            $key = $_
            if ($Config.retentionRules.PSObject.Properties.Name -contains $key) {
                $v = $Config.retentionRules.$key
                if (-not ($v -is [int]) -or $v -lt 0) {
                    $errors += ("retentionRules.{0} must be non-negative integer, got: {1}" -f $key, $v)
                }
            }
        }
    }
    if ($Config.PSObject.Properties.Name -contains 'communication') {
        @('bridgeEnabled','requirePermissionForHighRisk') | ForEach-Object {
            $key = $_
            if ($Config.communication.PSObject.Properties.Name -contains $key) {
                if ($Config.communication.$key -isnot [bool]) {
                    $errors += ("communication.{0} must be boolean" -f $key)
                }
            }
        }
    }
...
function Set-GuardianRuntimeConfigValidated {
    param([Parameter(Mandatory)]$Config)
    $v = Test-GuardianRuntimeConfig -Config $Config
    if (-not $v.valid) {
        if (Get-Command Write-GuardianAudit -ErrorAction SilentlyContinue) {
            Write-GuardianAudit -Action 'runtime_config_rejected' -Reason ($v.errors -join '; ') -NewState 'rejected' -Validation 'failed' | Out-Null
        }
...
function Sync-GuardianSchedulerFromConfig {
    if (-not (Test-Path $GuardianRuntimeConfig)) { return $false }
    $cfg = Get-Content -Path $GuardianRuntimeConfig -Encoding UTF8 | ConvertFrom-Json
    if (-not $cfg.PSObject.Properties.Name.Contains('schedulerIntervals')) { return $false }
    $cfg.schedulerIntervals.PSObject.Properties | ForEach-Object {
        $j = Get-GuardianJob -Name $_.Name
        if ($j.Count -gt 0) { Register-GuardianJob -Name $_.Name -IntervalSeconds $_.Value -Enabled $true | Out-Null }
    }
...
function Send-GuardianRiskReportToNexus98Bridge {
    param([string]$RiskLevel='low', [string]$Reason='')
    $body = [PSCustomObject]@{ message_id = "RISK_$(Get-Date -Format 'yyyyMMdd_HHmmss')_$(Get-Random -Maximum 999999)"; timestamp=(Get-Date).ToString('o'); kind='risk'; riskLevel=$RiskLevel; reason=$Reason }
    if (Get-Command Send-GuardianWarningToNexus98 -ErrorAction SilentlyContinue) { Send-GuardianWarningToNexus98 -Warning $Reason -RiskLevel $RiskLevel | Out-Null }
    if (Get-Command Write-GuardianAudit -ErrorAction SilentlyContinue) { Write-GuardianAudit -Action 'comms_risk' -Reason 'risk report to Nexus98' -NewState $body.message_id -Validation 'sent' | Out-Null }
    return [PSCustomObject]@{ sent=$true; message_id=$body.message_id; kind='risk' }
}

function Send-GuardianCheckpointNoticeToNexus98Bridge {
    param([Parameter(Mandatory)]$CheckpointId, [string]$Tier='rolling', [string]$Reason='routine')
    $body = [PSCustomObject]@{ message_id = "CKPT_$(Get-Date -Format 'yyyyMMdd_HHmmss')_$(Get-Random -Maximum 999999)"; timestamp=(Get-Date).ToString('o'); kind='checkpoint'; checkpointId=$CheckpointId; tier=$Tier; reason=$Reason }
    if (Get-Command Send-GuardianRecommendationToNexus98 -ErrorAction SilentlyContinue) { Send-GuardianRecommendationToNexus98 -Recommendation "Checkpoint $CheckpointId ($Tier) created: $Reason" | Out-Null }
    if (Get-Command Write-GuardianAudit -ErrorAction SilentlyContinue) { Write-GuardianAudit -Action 'comms_checkpoint' -Reason 'checkpoint notice to Nexus98' -NewState $CheckpointId -Validation 'sent' | Out-Null }
    return [PSCustomObject]@{ sent=$true; message_id=$body.message_id; kind='checkpoint'; checkpointId=$CheckpointId }
}

function Send-GuardianRollbackNoticeToNexus98Bridge {
    param([Parameter(Mandatory)]$CheckpointId, [string]$Reason='rollback')
    $body = [PSCustomObject]@{ message_id = "RBAC_$(Get-Date -Format 'yyyyMMdd_HHmmss')_$(Get-Random -Maximum 999999)"; timestamp=(Get-Date).ToString('o'); kind='rollback'; checkpointId=$CheckpointId; reason=$Reason }
    if (Get-Command Send-GuardianRecommendationToNexus98 -ErrorAction SilentlyContinue) { Send-GuardianRecommendationToNexus98 -Recommendation ("Rollback to " + $CheckpointId + ": " + $Reason) | Out-Null }
    if (Get-Command Write-GuardianAudit -ErrorAction SilentlyContinue) { Write-GuardianAudit -Action 'comms_rollback' -Reason 'rollback notice to Nexus98' -NewState $CheckpointId -Validation 'sent' | Out-Null }
    return [PSCustomObject]@{ sent=$true; message_id=$body.message_id; kind='rollback'; checkpointId=$CheckpointId }
}

function Get-GuardianHealthExplanation {
    $health = Get-GuardianHealthScore
    $explain = [PSCustomObject]@{
        overallPct = $health.overallPct
        status = if ($health.overallPct -ge 80) { 'healthy' } elseif ($health.overallPct -ge 50) { 'degraded' } else { 'critical' }
        reasons = @("architecture coverage=$($health.architecturePct)% ($($health.subsystemsPresent)/$($health.subsystemsTotal) present) -> weight 60%", "storage hygiene=$($health.storageHygienePct)% (snapshot files=$($health.snapshotFiles), ~$($health.snapshotMB) MB) -> weight 40%")
    }
    $comm = if (Get-Command Get-GuardianCommunicationHealth -ErrorAction SilentlyContinue) { Get-GuardianCommunicationHealth } else { $null }
    if ($comm) { $explain.reasons += ("communication health=$($comm.healthScore) successRate=$($comm.successRatePct)% bridgeEnabled=$($comm.enabled)") }
    return $explain
}

function Invoke-GuardianCheckpointLifecycle {
    param([int]$KeepRolling=25, [switch]$ArchiveExcess)
    $rolling = @(Get-GuardianCheckpoints -Tier rolling) | Sort-Object { $_.created }
    $total = $rolling.Count
    $excess = [math]::Max(0, $total - $KeepRolling)
    $action = if ($ArchiveExcess) { 'archived' } else { 'reported' }
    $rotated = @()
    if ($excess -ge $total -and $total -gt 0) { $excess = $total - 1 }
    if ($excess -gt 0) {
        $rolling | Select-Object -First $excess | ForEach-Object {
            ...
            $rotated += [PSCustomObject]@{ id=$_.id; tier='rolling'; action=$action }
        }
    }
    $milestones = @(Get-GuardianCheckpoints -Tier milestones).Count
    $result = [PSCustomObject]@{
        rollingTotal=$total; keepRolling=$KeepRolling; rotated=$excess; details=$rotated
        milestones=$milestones
        recoveryReadiness=if($total -gt 0){'ready'}else{'none'}
        onlyRecoveryPointProtected=($total -le $KeepRolling)
        timestamp=(Get-Date).ToString('o')
    }
    if (Get-Command Write-GuardianAudit -ErrorAction SilentlyContinue) { Write-GuardianAudit -Action 'checkpoint_lifecycle' -Reason "retention keep=$KeepRolling rotated=$excess" -NewState $action -Validation 'completed' | Out-Null }
    return $result
}

function Invoke-GuardianStorageGovernanceReview {
    $findings = @(Get-GuardianStorageGovernance -Root $GuardianEnv.Root)
    $byRule = @{}
    foreach ($f in $findings) {
        $cat = if ($f.lifecycle -and $f.lifecycle.category) { $f.lifecycle.category } else { 'UNKNOWN' }
        if (-not $byRule.ContainsKey($cat)) { $byRule[$cat] = 0 }
        $byRule[$cat]++
    }
    $recommendations = @()
    foreach ($f in $findings) {
    ...
    if (Get-Command New-GuardianEvent -ErrorAction SilentlyContinue) {
        New-GuardianEvent -Source 'storage' -Category FILE_SYSTEM -Severity $(if($findings.Count -gt 0){'WARNING'}else{'INFO'}) -Description "STORAGE_GOVERNANCE_REVIEW findings=$($findings.Count)" -AffectedComponent 'storage' | Write-GuardianEvent | Out-Null
    }
    return $result
}
